import React, { useState } from 'react';
import { useAuth } from '../Auth/AuthProvider';
import styles from './ChapterActions.module.css';

interface TranslateButtonProps {
  chapterId: string;
  chapterTitle: string;
}

const TranslateButton: React.FC<TranslateButtonProps> = ({ chapterId, chapterTitle }) => {
  const { user } = useAuth();
  const [isTranslated, setIsTranslated] = useState(false);
  const [loading, setLoading] = useState(false);

  if (!user) {
    return null; // Don't show button if not logged in
  }

  const handleTranslate = async () => {
    setLoading(true);
    
    try {
      if (!isTranslated) {
        // Simulate translation API call
        await translateContent(chapterId);
        setIsTranslated(true);
        showNotification('🇵🇰 Content translated to Urdu');
      } else {
        // Switch back to English
        setIsTranslated(false);
        showNotification('🇬🇧 Showing English content');
      }
      
      // Store preference
      localStorage.setItem(`translated_${chapterId}`, (!isTranslated).toString());
    } catch (error) {
      showNotification('❌ Translation failed. Please try again.');
    } finally {
      setLoading(false);
    }
  };

  const translateContent = async (chapterId: string) => {
    // In production, this would call your translation API
    // For now, simulate delay
    await new Promise(resolve => setTimeout(resolve, 2000));
    
    // Here you would:
    // 1. Call OpenAI API to translate
    // 2. Cache translation
    // 3. Replace page content
    
    return true;
  };

  return (
    <button
      className={`${styles.actionButton} ${styles.translateButton} ${isTranslated ? styles.active : ''}`}
      onClick={handleTranslate}
      disabled={loading}
    >
      <svg 
        className={styles.icon} 
        width="20" 
        height="20" 
        viewBox="0 0 20 20"
        fill="none"
        xmlns="http://www.w3.org/2000/svg"
      >
        <path 
          d="M12.87 15.07l-2.54-2.51.03-.03c1.74-1.94 2.98-4.17 3.71-6.53H17V4h-7V2H8v2H1v2h11.17C11.5 7.92 10.44 9.75 9 11.35 8.07 10.32 7.3 9.19 6.69 8h-2c.73 1.63 1.73 3.17 2.98 4.56l-5.09 5.02L4 19l5-5 3.11 3.11.76-2.04zM18.5 10h-2L12 22h2l1.12-3h4.75L21 22h2l-4.5-12zm-2.62 7l1.62-4.33L19.12 17h-3.24z" 
          fill="currentColor"
        />
      </svg>
      
      <div className={styles.buttonContent}>
        <span className={styles.buttonTitle}>
          {isTranslated ? '🇵🇰 اردو میں' : '🌍 Translate to Urdu'}
        </span>
        <span className={styles.buttonSubtitle}>
          {isTranslated 
            ? 'انگریزی میں دیکھیں' 
            : 'Read this chapter in Urdu'}
        </span>
      </div>
      
      {loading && (
        <div className={styles.spinner}></div>
      )}
    </button>
  );
};

function showNotification(message: string) {
  const notification = document.createElement('div');
  notification.className = styles.notification;
  notification.textContent = message;
  document.body.appendChild(notification);
  
  setTimeout(() => notification.classList.add(styles.show), 10);
  
  setTimeout(() => {
    notification.classList.remove(styles.show);
    setTimeout(() => notification.remove(), 300);
  }, 3000);
}

export default TranslateButton;
